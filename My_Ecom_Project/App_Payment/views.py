...
# Payments
import requests
# from sslcommerz_python.payment import SSLSession
...
def checkout(request):
    ...
    return render(request, "App_Payment/checkout.html", context=diction)
...
def payment(request):
    ...
    status_url = requests.build_absolute_uri(reverse("App_Payment:complete"))
    print(status_url)
    mypayment.set_urls(success_url=status_url, fail_url=status_url, cancel_url=status_url, ipn_url=status_url)
    
    order_qs = Order.objects.filter(user=request.user, ordered=False)
    order_items = order_qs[0]
    ...
    mypayment.set_customer_info(
        name=current_user.profile.full_name, email=current_user.email, address1=current_user.profile.address_1,
    ...
    mypayment.set_shipping_info(
        shipping_to=current_user.profile.full_name, address=saved_address.address, 
